...
export async function user_data(req, res) {
    ...
    try {
        const usuario = await user.findOne({ id: req.body.id }, { nombre: 1, correo: 1, _id: 0 });
        console.log(usuario)
        res.json(usuario);
    } catch (err) {
...
export async function user_login(req, res) {
    ...
    try {
        const usuario = await user_log.findOne({ id: req.body.id }, { id: 1, contraseña: 1, verificado: 1, _id: 0 });
        console.log(usuario);
